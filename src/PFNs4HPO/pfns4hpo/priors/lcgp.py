...
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, covar_module, mean_module, num_tasks):
        super(MultitaskGPModel, self).__init__(train_x, train_y, likelihood)
        
        self.mean_module = gpytorch.means.MultitaskMean(
            mean_module, num_tasks=num_tasks
        )
        self.covar_module = gpytorch.kernels.MultitaskKernel(
            covar_module, num_tasks=num_tasks, rank=1
        )
...
def get_model(x, hyperparameters: dict):
    ...
    model = MultitaskGPModel(train_x=x, 
                            train_y=torch.zeros_like(x[...,0]), 
...
def sample_curves(max_epochs, nb_curves, num_features, unused_feature_likelihood, device, hyperparameters):

    configurations = torch.rand(nb_curves, num_features, device=device)
            ...
            with gpytorch.settings.prior_mode(True):
                try:
                    with warnings.catch_warnings():
                        ...
                        input_warp.sample_from_prior("concentration1_prior")
                        input_warp.sample_from_prior("concentration0_prior")
                        _model = model.pyro_sample_from_prior()

                        wrap_config = input_warp(used_local_x)
                        d = _model(wrap_config)
                        sample = d.sample()
                ...
                t = torch.arange(1, max_epochs + 1) 
                bf = bf_layer(torch.tanh(sample[:, :9]), t)
                weights = torch.softmax(sample[:, 9:12], -1).unsqueeze(-1)
                y = torch.tanh(torch.sum(bf * weights, dim=(1,), keepdim=False)) + torch.sigmoid(sample[:, -2:-1])
                sigma = torch.sigmoid(sample[:, -1])
                sigma = (sigma - sigma.min() + 1e-5) / torch.randint(1, 100, (1,))[0]
                y = (y + torch.normal(torch.zeros_like(y), sigma.unsqueeze(1).repeat(1, max_epochs)))

            return configurations, y
...
def get_batch(batch_size, seq_len, num_features, device=default_device, hyperparameters=None,
    ...
    for idx_batch in range(batch_size):
        ...
        while not ok:
            configurations, curves = sample_curves(max_epochs, nb_curves, num_params, unused_feature_likelihood, device, hyperparameters)
