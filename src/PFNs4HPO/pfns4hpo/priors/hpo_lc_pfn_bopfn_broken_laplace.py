...
import numpy as np
import random
from scipy.stats import norm, beta, gamma, expon, laplace
...
class DatasetPrior:
    ...
    def curves_for_configs(self, configs, noise=True):
        ...
        Rpsat = np.stack([rng4config.laplace(loc=0.5,scale=0.25) for i in range(ncurves)]).T # 18, 19, 20, 21
        #Rpsat = np.stack([1.0 for i in range(ncurves)]).T # 18, 19, 20, 21
    ...
    def laplace(self, bnn_output, loc=0, scale=1):
        eps = 0.5 / len(DatasetPrior.output_sorted) # to avoid infinite samples
        u = self.uniform(bnn_output, a=eps, b=1-eps)
        return laplace.ppf(u, loc=loc, scale=scale)
...
class MyRNG:
    ...
    def laplace(self, loc=0, scale=1):
        eps = 0.5 / len(DatasetPrior.output_sorted) # to avoid infinite samples
        u = self.uniform(a=eps, b=1-eps)
        return laplace.ppf(u, loc=loc, scale=scale)
