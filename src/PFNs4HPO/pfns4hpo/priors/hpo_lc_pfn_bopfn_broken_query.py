    ...
    for i in range(batch_size):
        ...
        for i in range(single_eval_pos): # loop over every pos
            ...
            if cutoff_per_curve[cid] >= dyn_max:
                dyn_max += 1
        # select the queries for each curve
        cand_queries = []
        p_queries = []
        for cid in ids: # loop over every curve
            ...
            for t in range(cutoff+1, n_levels+1): # loop over future query epochs
                cand_queries.append((cid,t))
                # (sqrt(data_at) + cutoff + 1) / (t + 1) + I(at_max) + I(at_dyn_max)
                pval = 0.0
                #pval += (np.sqrt(ncurves_at[t-1])+1)/(np.sqrt(single_eval_pos)+t) 
                pval += (np.sqrt(ncurves_at[t-1]) + cutoff**2 + cutoff + 1)/(np.sqrt(ncurves_at[t-1]) + cutoff**2 + t)
                pval += (1 if t==dyn_max else 0) # dyna
                pval += (1 if t==n_levels else 0) # at max
                pval *= (0.1 if cutoff == 0 else 1.0)
                p_queries.append(pval)
        p_queries = np.array(p_queries)
        p_queries /= p_queries.sum()
        cand_queries = np.array(cand_queries)
