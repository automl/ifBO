"""
CHANGELOG: Extend the curve prior to support
- multiple basis curves (4 subsuming the 11 by Domhan et al)
- breaking points and divergence
- heterogeneous noise
"""
...
class DatasetPrior:
    ...
    def rngs_for_configs(self, configs, noise=True):
        ...
        return [MyRNG(ids) for ids in indices]
...
def curve_prior(dataset, config, rng4config=None):
    ncurves = 4

    if rng4config is None:
        # best do this in batch as it is much more efficient
        rng4config = dataset.rngs_for_configs(np.array([config]))[0]
    ...
    w = [rng4config.gamma(a=1) for i in range(ncurves)] # 1, 2, 3, 4
    w_sum = sum(w)
    w = [wi/w_sum for wi in w]

    # sample shape/skew parameter for each basis curve
    alpha = [np.exp(rng4config.normal(1,1)), # 5
             np.exp(rng4config.normal(0,1)), # 6
             1.0+np.exp(rng4config.normal(-4,1)), # 7
             np.exp(rng4config.normal(0.5,0.5))] # 8

    # sample saturation x for each basis curve
    Xsat_max = 10**rng4config.normal(0,1)  # max saturation # 9
    Xsat_rel = np.array([rng4config.gamma(a=1) for i in range(ncurves)]) # relative saturation points # 10, 11, 12, 13
    Xsat = Xsat_max * Xsat_rel / np.max(Xsat_rel)
    #Xsat = [Xsat_max, Xsat_max, Xsat_max, Xsat_max]
    
    # sample relative saturation y (PREC) for each basis curve
    PREC = [1.0 / 10**rng4config.uniform(-3,0) for i in range(ncurves)] # 14, 15, 16, 17

    # post saturation convergence/divergence rate for each basis curve
    Rpsat = [1.0 - rng4config.exponential(scale=1) for i in range(ncurves)] # 18, 19, 20, 21
    ...
    def foo(x_):
        y_ = comb(x_, Y0=Y0, Yinf=Yinf, sigma=sigma, L=L, Xsat=Xsat, alpha=alpha, Rpsat=Rpsat, w=w)
        return y_
...
def get_batch(
    ...
    **kwargs,
    ...
    dataset_prior = DatasetPrior(num_params, 24)
    ...
    for i in range(batch_size):
        ...
        rng_configs = dataset_prior.rngs_for_configs(curve_configs)
        curve_xs = []
        curve_ys = []
        for cid in range(seq_len): # loop over every curve
            if epochs_per_curve[cid] > 0:
                ...
                curve_xs.append(x_)
                # sample curve
                curve = curve_prior(dataset_prior,c,rng_configs[cid])
                # determine y's
