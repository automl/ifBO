...
import numpy as np
import random
from scipy.stats import norm, beta, gamma, expon, truncnorm
...
def truncated_multivariate_gaussian_sample(mean, std_dev, lower_bound, upper_bound):
    num_dimensions = len(mean)

    # Calculate the lower and upper bounds for the standard normal distribution
    a_std = (lower_bound - mean) / std_dev
    b_std = (upper_bound - mean) / std_dev

    # Generate samples from the truncated normal distribution for each dimension
    samples = truncnorm.rvs(a_std, b_std, loc=mean, scale=std_dev, size=num_dimensions)
...
class DatasetPrior:
    ...
    def new_dataset(self):
        ...
        u = [np.random.uniform() for _ in range(4)]
        self.y0 = min(u)
        self.ymax = max(u)
    ...
    def curves_for_configs(self, configs, noise=True):
        ...
        def foo(x_, cid=0):
            ...
            y_noisy = truncated_multivariate_gaussian_sample(y_, sigma[cid], 0.0, 1.0)
            return y_noisy
        return foo
...
def get_batch(
    ...
    **kwargs,
    ...
    for i in range(batch_size):
        ...
        p_levels = np.array([np.power(0.995,i) for i in range(seq_len)])
        n_levels = np.random.choice(seq_len,p=p_levels/p_levels.sum()) + 1
        ...
        ordering = np.random.choice(all_levels, p=all_p, size=single_eval_pos, replace=False)

        # calculate the cutoff for each curve
        cutoff_per_curve = np.zeros((seq_len,), dtype=int)
        ncurves_at = np.zeros((n_levels,), dtype=int)
        dyn_max = 1
        for i in range(single_eval_pos): # loop over every pos
            cid = ordering[i]
            ncurves_at[cutoff_per_curve[cid]] += 1
            cutoff_per_curve[cid] += 1
            if cutoff_per_curve[cid] >= dyn_max:
                dyn_max += 1
        #print(ncurves_at)
        # select the queries for each curve (50% new and 50% extensions of partial)
        cand_queries_partial = []
        cand_queries_new = []
        p_queries_partial = []
        p_queries_new = []
        for cid in ids: # loop over every curve
            cutoff = cutoff_per_curve[cid]
            for t in range(cutoff+1, n_levels+1): # loop over future query epochs
                pval = 1.0 / np.sqrt(ncurves_at[cutoff-1]) if cutoff > 0 else 1.0 / np.sqrt(seq_len)
                pval *= ((1 if t==n_levels else 0)  + (np.sqrt(ncurves_at[t-1]) + cutoff**2 + cutoff + 1)/(np.sqrt(ncurves_at[t-1]) + cutoff**2 + t))
                if cutoff == 0:
                    cand_queries_new.append((cid,t))
                    p_queries_new.append(pval)
                else:
                    cand_queries_partial.append((cid,t))
                    p_queries_partial.append(pval)
        p_queries_partial = np.array(p_queries_partial)
        p_queries_new = np.array(p_queries_new)
        cand_queries_partial = np.array(cand_queries_partial)
        cand_queries_new = np.array(cand_queries_new)
        if len(cand_queries_new) == 0:
            p_queries = p_queries_partial / p_queries_partial.sum()
            cand_queries = cand_queries_partial
            p_queries = np.concatenate([p_queries_partial, p_queries_new])
        elif len(cand_queries_partial) == 0:
            p_queries = p_queries_new / p_queries_new.sum()
            cand_queries = cand_queries_new
        else:
            #p_queries_new /= (2*p_queries_new.sum())
            #p_queries_partial /= (2*p_queries_partial.sum())
            cand_queries = np.concatenate([cand_queries_partial, cand_queries_new])
            p_queries = np.concatenate([p_queries_partial, p_queries_new])
            p_queries /= p_queries.sum()

        query_ids = np.random.choice(len(cand_queries), p=p_queries, size=seq_len-single_eval_pos, replace=False)
        queries = cand_queries[query_ids]

        # calculate the x for each curve
        curve_xs = []
        # x train
        for cid in range(seq_len): # loop over every curve
            cutoff = cutoff_per_curve[cid]
            curve_xs.append(list(v/n_levels for v in range(1, cutoff+1)))
        # x test/queries
        for cid, t in queries:
            curve_xs[cid].append(t/n_levels)

        # fix dataset specific random variables
        dataset_prior.new_dataset()

        # determine config, y for every curve
        curve_configs = np.random.uniform(size=(seq_len, num_params))
        curves = dataset_prior.curves_for_configs(curve_configs)
        curve_ys = []
        for cid in range(seq_len): # loop over every curve
            if len(curve_xs[cid]) > 0:
                x_ = np.array(curve_xs[cid])
                curve_xs[cid] = x_
                # determine y's
                y_ = curves(x_, cid)
                curve_ys.append(y_)
        ...
        for i in range(seq_len):
            if i < single_eval_pos:
                ...
                cid = queries[i-single_eval_pos][0]
