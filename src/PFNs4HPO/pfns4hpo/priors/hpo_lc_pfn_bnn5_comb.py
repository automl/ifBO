...
def comb(x, Y0=0.2, Yinf=0.8, sigma=0.01, L=0.0001, PREC=[100]*4, Xsat=[1.0]*4, alpha=[np.exp(1), np.exp(-1), 1+np.exp(-4), np.exp(0)], Rpsat=[1.0]*4, w=[1/4]*4):
    ...
    x_exp = x # add_noise_and_break(x, x_noise, Xsat[1], Rpsat[1])
    exp_x = PREC[1]**(-(x_exp/Xsat[1])**alpha[1])
    
    x_log = x #add_noise_and_break(x, x_noise, Xsat[2], Rpsat[2])
    log_x = np.log(alpha[2])/(np.log((alpha[2]**PREC[2] - alpha[2]) * x_log/Xsat[2] + alpha[2]))

    x_hill = x # add_noise_and_break(x, x_noise, Xsat[3], Rpsat[3])
...
class DatasetPrior:
    ...
    def new_dataset(self):
        ...
        self.y0 = min(u1,u2)
        self.ymax = max(u1,u2) if np.random.uniform() < 0.5 else 1.0
    ...
    def curves_for_configs(self, configs, noise=True):
        ...
        def foo(x_, cid=0):
            y_ = comb(x_, Y0=Y0, Yinf=Yinf[cid], sigma=None, L=None, Xsat=Xsat[cid], alpha=alpha[cid], Rpsat=None, w=w[cid], PREC=PREC[cid])
...
def get_batch(
    ...
    **kwargs,
    ...
    dataset_prior = DatasetPrior(num_params, 19)
