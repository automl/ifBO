...
import json
import torch
from pfns4hpo import priors, utils, train
from pfns4hpo import bar_distribution, encoders
...
def main(configs):
    ...
    if configs["load_path"] is None:
        ...
        if configs["num_gpus"] == 1:
            prior_data = priors.utils.PriorDataLoader(configs["load_path"],subsample=configs["subsample"],n_chunks=configs["n_chunks"])
        else:
            prior_data = priors.utils.DistributedPriorDataLoader(configs["load_path"],subsample=configs["subsample"],n_chunks=configs["n_chunks"], n_gpus=configs["num_gpus"])
        get_batch_func = lambda *args, **kwargs: prior_data.get_batch(kwargs.get("device", args[4] if len(args) >= 5 else "cpu"))

    num_features = configs["num_features"]

    if configs["use_buckets_file"] is not None:
        bucket_limits = torch.load(configs["use_buckets_file"])
    elif configs["linspace_borders"]:
        ...
        while offset < configs["border_batch_size"]:
            ...
            ys_bucket[:,offset:min(offset+eff_batch_size, configs["border_batch_size"])] = ys.target_y[:,:min(eff_batch_size, configs["border_batch_size"]-offset)]
    ...
    configs_train = {_: configs[_]
                     for _ in
                     ["nlayers", "emsize", "epochs", "lr", "nhead",
                      "aggregate_k_gradients", "bptt",
                      "steps_per_epoch", "train_mixed_precision",
                      "batch_size"]}
    ...
    configs_train["warmup_epochs"] = (configs["epochs"] // 4 if configs["warmup_epochs"] == -1 else configs["warmup_epochs"])
    if configs["load_path"] is None:
    ...
    executor = submitit.get_executor(
        folder="/tmp",
        timeout_min=configs["time"]
    )
    if configs["continue_model"] is not None:
        configs_train["continue_model"] = executor.get_group(configs["continue_model"])[0].results()[0][2]

    if configs["run_on_submitit"]:
        executor.update_parameters(
            slurm_time=configs["time"],
            slurm_partition=configs["partition"],
            nodes=1,
            cpus_per_task=configs["num_gpus"],
            slurm_ntasks_per_node=configs["num_gpus"],
            slurm_gres="gpu:{0}".format(configs["num_gpus"]),
            slurm_exclude="dlcgpu16"
        )
        print(configs["name"], configs_train)
        job_group = executor.submit_group(
            configs["name"], train.train, [configs_train])
        print(job_group)
    else:
        train.train(**configs_train)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--priordataloader_class', type=str, help='Class for Prior Data Loader', default="test")
    # parser.add_argument('--style_encoder_generator', type=str, required=True, help='Style Encoder Generator Function')
    # parser.add_argument('--criterion', type=str, required=True, help='Criterion Function')
    # parser.add_argument('--encoder_generator', type=str, required=True, help='Encoder Generator Function')
    parser.add_argument('--warmup_epochs', type=int, required=False, default=-1, help='Number of Warmup Epochs')
    # parser.add_argument('--y_encoder_generator', type=str, required=True, help='y Encoder Generator Function')
    # parser.add_argument('--scheduler', type=str, required=True, help='Learning Rate Scheduler')
    # parser.add_argument('--nhead', type=int, required=True, help='Number of Heads in Attention Mechanism')
    ...
    parser.add_argument(
        '--num_borders', type=int, default=10000,
    ...
    parser.add_argument(
        '--max_epochs_per_curve', type=int, default=50,
    ...
    parser.add_argument(
        '--time', type=int, default=23 * 60 + 55,
        help='Time budget (min) for the training')
    parser.add_argument(
        '--partition',
        type=str,
        default="alldlc_gpu-rtx2080",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help='Continue from a previously trained model',
        default=None)
    parser.add_argument(
        '--use_buckets_file',
        type=str,
        required=False,
        help='File path to load the buckets. Load with torch.load',
        default=None)
    parser.add_argument(
        '--model_extra_args',
        type=str,
    ...
    if args.run_on_submitit:
        # If --train_mixed_precision is True, then --name must be set
        if args.name is None:
            parser.error(
                "--name is required when --run_on_submitit is set to True")
    print(args)
