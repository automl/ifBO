...
class StyleEncoder(nn.Module):
    def __init__(self, num_hyperparameters: int, em_size: int) -> None:
        super().__init__()
        self.em_size = em_size
        self.embedding = nn.Linear(num_hyperparameters, self.em_size)

    def forward(self, hyperparameters: dict[str, Any]) -> torch.Tensor:  # B x num_hps
        return self.embedding(hyperparameters)


class StyleEmbEncoder(nn.Module):
    def __init__(self, num_hyperparameters: int, em_size: int, num_embeddings: int = 100) -> None:
        super().__init__()
        assert num_hyperparameters == 1
        self.em_size = em_size
        self.embedding = nn.Embedding(num_embeddings, self.em_size)

    def forward(self, hyperparameters: torch.Tensor) -> torch.Tensor:  # B x num_hps
...
class _PositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # T x B x num_features
        ...
        div_term = (
            ...
            * math.pi
            * torch.exp(
                torch.arange(0, d_per_feature, 2, device=self.device_test_tensor.device).float()
...
class Positional(_PositionalEncoding):
    def __init__(self, num_features: int, emsize: int) -> None:
        super().__init__(d_model=emsize)
        self.num_features = num_features


class EmbeddingEncoder(nn.Module):
    def __init__(self, num_features: int, em_size: int, num_embs: int = 100) -> None:
        super().__init__()
    ...
    def init_weights(self, initrange: float) -> None:
...
def get_normalized_uniform_encoder(
    encoder_creator: Callable[[int, int], nn.Module],
...
def get_normalized_encoder(
    encoder_creator: Callable[[int, int], nn.Module], data_std: float
) -> Callable[[int, int], nn.Module]:
    return lambda in_dim, out_dim: nn.Sequential(
...
def get_log_dims(x: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
    logged_x = ((x + eps).log() - math.log(eps)) / (math.log(1.0 + eps) - math.log(eps))
    return logged_x


def add_log_neglog_dims(x: torch.Tensor, eps: float = 1e-10) -> torch.Tensor:
...
def get_logdim_encoder(
    encoder_creator: Callable[[int, int], nn.Module], eps: float = 1e-10
) -> Callable[[int, int], nn.Module]:
    return lambda in_dim, out_dim: nn.Sequential(
...
class AppendEmbeddingEncoder(nn.Module):
    def __init__(
        self, base_encoder: Callable[[torch.Tensor], torch.Tensor], num_features: int, emsize: int
    ) -> None:
        super().__init__()
        self.num_features = num_features
...
def get_append_embedding_encoder(
    encoder_creator: Callable[[int, int], nn.Module],
) -> Callable[[int, int], nn.Module]:
...
class VariableNumFeaturesEncoder(nn.Module):
    def __init__(
        self, base_encoder: Callable[[torch.Tensor], torch.Tensor], num_features: int
    ) -> None:
        super().__init__()
        self.base_encoder = base_encoder
        self.num_features = num_features
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat(
            (x, torch.zeros(*x.shape[:-1], self.num_features - x.shape[-1], device=x.device)), -1
        )
        return self.base_encoder(x)


def get_variable_num_features_encoder(
    encoder_creator: Callable[[int, int], nn.Module],
) -> Callable[[int, int], nn.Module]:
...
class NoMeanEncoder(nn.Module):
    ...
    def __init__(self, base_encoder: Callable[[torch.Tensor], torch.Tensor]) -> None:
        super().__init__()
        self.base_encoder = base_encoder
...
def MLP(num_features: int, emsize: int) -> nn.Sequential:
    return nn.Sequential(
        nn.Linear(num_features, emsize * 2),
        nn.ReLU(),
...
class NanHandlingEncoder(nn.Module):
    def __init__(self, num_features: int, emsize: int, keep_nans: bool = True) -> None:
        super().__init__()
...
class Linear(nn.Linear):
    def __init__(self, num_features: int, emsize: int, replace_nan_by_zero: bool = False) -> None:
...
class Conv(nn.Module):
    def __init__(self, input_size: int, emsize: int) -> None:
        super().__init__()
...
class CanEmb(nn.Embedding):
    def __init__(
        self, num_features: int, num_embeddings: int, embedding_dim: int, *args: Any, **kwargs: Any
    ) -> None:
...
def get_Canonical(num_classes: int) -> Callable[[int, int], nn.Module]:
    return lambda num_features, emsize: CanEmb(num_features, num_classes, emsize)


def get_Embedding(num_embs_per_feature: int = 100) -> Callable[[int, int], nn.Module]:
